(function ($) {
    ...
    (function ($) {
        // Guard: ensure counterUp exists
        if (typeof $.fn.counterUp !== 'function') {
            console.warn('counterUp plugin not found. Make sure it is loaded before this script.');
            return;
        }

        var countersSelector = '.counter_area';
        var hasCountedClass = 'counters-animated'; // mark once animated

        function runCounters($container) {
            if ($container.hasClass(hasCountedClass)) return; // already done
            $container.addClass(hasCountedClass);
            $container.find('.counter').counterUp({
                delay: 10,
                time: 1000
            });
        }
            ...
            var observer = new IntersectionObserver(function (entries, obs) {
                entries.forEach(function (entry) {
                    if (entry.isIntersecting) {
                        ...
                        obs.unobserve(entry.target); // run once, then stop observing
                    }
                });
            }, observerOptions);

            // Observe every matching section (in case you have more than one)
            $(countersSelector).each(function () {
                observer.observe(this);
            });

        } else {
            // Fallback for older browsers: on scroll check (debounced)
            var $win = $(window);
            var $sections = $(countersSelector);
            var ticking = false;

            function isInViewport($el, pctVisible) {
                ...
                return (visibleHeight / elHeight) >= pctVisible;
            }

            function checkSections() {
                $sections.each(function () {
                    var $sec = $(this);
                    if (!$sec.hasClass(hasCountedClass) && isInViewport($sec, 0.4)) {
                        runCounters($sec);
                    }
                });
            }
